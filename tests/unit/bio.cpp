...
TEST_CASE("base", "[bio]") {
    uint8_t data[sizeof(kon_bio_test::foo)] = {0x78, 0x56, 0x34, 0x12, 0x34, 0x12, 0x78, 0x56};
    SECTION("read") {
        kon::bio<kon_bio_test::foo> view{data};
    ...
    SECTION("write") {
        std::uint8_t buffer[sizeof(kon_bio_test::foo)];
        kon::bio<kon_bio_test::foo> view{data};
        ...
        REQUIRE(std::memcmp(data, buffer, sizeof(data)));
    }
}
